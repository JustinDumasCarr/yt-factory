...
def logs_view_cmd(
    ...
    step: str = typer.Option(None, "--step", "-s", help="Filter to specific step (plan, generate, review, render, upload)"),
        ...
        if entries:
            ...
            for entry in entries:
                if json_logs:
                    ...
                    context = {k: v for k, v in entry.items() if k not in ("timestamp", "step", "level", "message", "project_id")}
...
def logs_summary_cmd(
    project_id: str = typer.Argument(..., help="Project ID"),
    step: str = typer.Option(None, "--step", "-s", help="Show summary for specific step (plan, generate, review, render, upload)"),
) -> None:
        ...
        if durations.get("avg_ms"):
            ...
            if durations.get("by_provider"):
                ...
                for provider, stats in durations["by_provider"].items():
                    typer.echo(f"    {provider}: {stats['avg_ms']:.0f}ms avg ({stats['count']} calls)")
