...
class SoundbankEntry(BaseModel):
    ...
    license_type: str | None = None  # "CC0" | "CC-BY" | "Pixabay" | "Suno" | "manual" | "custom"
    license_url: str | None = None  # URL to license text/documentation
...
def _load_soundbank() -> Soundbank:
    ...
    try:
        with open(SOUNDBANK_JSON, encoding="utf-8") as f:
            data = json.load(f)
...
def get_sound(sound_id: str) -> SoundbankEntry | None:
    """
...
def _generate_via_suno(
    ...
    try:
        ...
        negative_tags = (
            "Song, Melody, Beat, Drums, Rhythm, Music, Composition, Tune, Harmony, Chord"
        )
...
def generate_sound(
    ...
    if source == "auto":
        ...
        raise ValueError(
            f"Invalid source: {source}. Must be 'freesound', 'pixabay', 'suno', or 'auto'"
        )
    ...
    # All sources failed
    raise RuntimeError(f"All sound sources failed. Last error: {last_error}") from last_error
